...
## Сценарий 4: Выборки и отчёты

**Цель:** Показать предустановленные выборки и статистические отчёты с возможностью экспорта и печати.

### 4.1. Пациенты без полиса ОМС (позитивный)

**Шаги:**
1. Перейти в **"Выборки и отчёты"**
...
### 4.4. Нагрузка по врачам (позитивный)

**Шаги:**
1. Выбрать "Нагрузка по врачам"

**Ожидаемый результат:**
- Таблица по каждому врачу: всего приёмов, завершено, отменено, неявок
- Применение: руководство оценивает загруженность персонала

### 4.5. Популярные диагнозы (позитивный)

**Шаги:**
1. Выбрать "Популярные диагнозы"

**Ожидаемый результат:**
- Топ-20 диагнозов с кодом МКБ-10 и количеством случаев
- Применение: анализ эпидемиологической обстановки

### 4.6. Популярные процедуры (позитивный)

**Шаги:**
1. Выбрать "Популярные процедуры"

**Ожидаемый результат:**
- Топ-20 процедур с количеством назначений и итоговым числом процедур
- Применение: планирование загрузки процедурных кабинетов

**Что демонстрируется:**
- Бизнес-логика в готовых выборках
- Статистические отчёты по ключевым показателям
- Удобство для конечного пользователя

---

## Сценарий 5: Экспорт данных и печать

**Цель:** Показать формирование отчётов для внешнего использования и вывод на печать.

### 5.1. Экспорт выборки в CSV (позитивный)

**Шаги:**
1. Перейти в **"Выборки и отчёты"**
2. Открыть любую выборку (например, "Нагрузка по врачам")
3. Нажать **"↓ Экспорт CSV"**
4. Открыть файл в Excel

**Ожидаемый результат:**
- Файл скачан с заголовком отчёта и датой формирования
- Данные разбиты по колонкам
- Кириллица отображается корректно

### 5.2. Печать отчёта (позитивный)

**Шаги:**
1. Открыть любую выборку с данными
2. Нажать **"🖨 Печать"**

**Ожидаемый результат:**
- Открывается диалог печати браузера
- Кнопки навигации скрыты — виден только заголовок и таблица
- Возможность сохранить как PDF

### 5.3. Экспорт пациентов из таблицы (позитивный)
...
- Телефоны и СНИЛС сохранены как текст (без потери нулей)

### 5.4. Отчёт по посещениям пациента (позитивный)
...
**Ожидаемый результат:**
- Файл с заголовком (ФИО пациента, дата формирования)
- История всех посещений с диагнозами и специальностями врачей

**Что демонстрируется:**
- Экспорт в CSV для всех типов отчётов
- Печать / сохранение в PDF
- Формирование персональных отчётов
- Корректная работа с кодировкой кириллицы
...
| 7-9 мин | Сценарий 3: Фильтры |
| 9-13 мин | Сценарий 4: Выборки и отчёты (6 карточек) |
| 13-15 мин | Сценарий 5: Экспорт CSV и печать |
| 15-17 мин | Сценарий 6-7: Мед. записи, справочники (кратко) |
| 17-20 мин | Ответы на вопросы |
